...
def main():
    img_dir = '/home/mohameys/datasets/artelingo/images/'
    split = 'test'
    annotations_file = f'/home/mohameys/datasets/artelingo/annotations/multilingual_{split}_metadata.csv'
    ...
    train_dataloader = DataLoader(train_artelingo, batch_size=32, shuffle=False)


    parser = get_parser()
    args = parser.parse_args(['--cfg-path', 'eval_configs/minigpt4_eval_bloomz.yaml'])
    cfg = Config(args)
    print('Done\nLoading model...')
...
# ###Assistant: '
    ...
    print('Done\nGenerating captions...')
    captions, art_styles, paintings, emotions, languages = [], [], [], [], []
    for i, sample in enumerate(tqdm(train_dataloader)):
        ...
        language = sample['language'] if 'language' in sample.keys() else None
        prompt = random.choice(model.prompt_list)
        img_embeds, atts_img = model.prompt_wrap(img_embeds, atts_img, prompt, language, emotion)
    ...
    df = pd.DataFrame({'caption': captions,
                       ...
                       'emotion': emotions,
                       'language': languages})
    df.to_csv('artelingo23_pretrain.csv', index=False)
